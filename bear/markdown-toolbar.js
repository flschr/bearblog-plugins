// ==UserScript==
// @name         Bear Blog Markdown Toolbar (V3)
// @version      0.3.6
// @description  Full Toolbar, Menu & Large Overlay-Counter
// @author       René Fischer
// @match        *://bearblog.dev/*
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    function createMarkdownToolbar($textarea) {
        ...
        dropdown.style.cssText = `display: none; position: absolute; top: 34px; right: 0; background: ${isDark ? '#01242e' : 'white'}; border: 1px solid ${isDark ? '#555' : '#ccc'}; border-radius: 4px; z-index: 1000; min-width: 140px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);`;
        ...
        // --- GROSSER CHARACTER COUNTER OVERLAY ---
        const counter = document.createElement('div');
        counter.id = 'char-counter-big';
        counter.style.cssText = `
            position: absolute; bottom: 20px; right: 30px; 
            padding: 5px 15px; border-radius: 10px; 
            font-size: 24px; font-weight: 800; font-family: ui-sans-serif, system-ui, sans-serif;
        ...
        const updateCounter = () => {
            ...
            if (len >= 300) {
                counter.style.background = '#d32f2f'; counter.style.color = 'white'; counter.style.borderColor = '#b71c1c';
            } else if (len >= 250) {
                counter.style.background = '#fbc02d'; counter.style.color = '#000'; counter.style.borderColor = '#f9a825';
            } else {
                counter.style.background = isDark ? '#01242e' : '#ffffff';
